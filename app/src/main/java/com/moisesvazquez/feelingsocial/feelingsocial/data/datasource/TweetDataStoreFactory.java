/**
 * Copyright (C) 2015 Fernando Cejas Open Source Project
 * <p>
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * <p>
 * http://www.apache.org/licenses/LICENSE-2.0
 * <p>
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package com.moisesvazquez.feelingsocial.feelingsocial.data.datasource;

import android.support.annotation.NonNull;

import com.moisesvazquez.feelingsocial.feelingsocial.data.cache.TweetCache;
import com.moisesvazquez.feelingsocial.feelingsocial.data.net.TwitterSdkRestApiImpl;
import com.moisesvazquez.feelingsocial.feelingsocial.domain.repository.datasource.TweetDataStore;

import javax.inject.Inject;
import javax.inject.Singleton;

import dagger.Lazy;

/**
 * Factory that creates different implementations of {@link TweetDataStore}.
 */
@Singleton
public class TweetDataStoreFactory {

    private final TweetCache tweetCache;

    @Inject
    Lazy<TwitterSdkRestApiImpl> restApi;

    @Inject
    public TweetDataStoreFactory(@NonNull TweetCache userCache) {
        this.tweetCache = userCache;
    }

    /**
     * Create {@link TweetDataStore}es  from a user id.
     */
    public TweetDataStore create(int tweetId) {
        TweetDataStore userDataStore;

        if (!this.tweetCache.isExpired() && this.tweetCache.isCached(tweetId)) {
            userDataStore = new DiskTweetDataStore(this.tweetCache);
        } else {
            userDataStore = createCloudDataStore();
        }

        return userDataStore;
    }

    /**
     * Create {@link TweetDataStore} to retrieve data from the Cloud.
     */
    public TweetDataStore createCloudDataStore() {
        return new CloudTweetDataStore(restApi.get(), this.tweetCache);
    }
}